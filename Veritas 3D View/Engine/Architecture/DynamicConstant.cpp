...
#undef X
		...
		default:
			WINRT_ASSERT("Bad type in signature generation" && false);
			return "???";
		}
	}
	...
	std::pair<size_t, const LayoutElement*> LayoutElement::CalculateIndexingOffset(size_t xoffset, size_t index) const noxnd
	{
		WINRT_ASSERT("Indexing into non-array" && type == Type::Array);
		const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
		WINRT_ASSERT(index < data.size);
		return { xoffset + data.element_size * index,&*data.layoutElement };
	}
	LayoutElement& LayoutElement::operator[](std::string_view key) noxnd
	{
		WINRT_ASSERT("Keying into non-struct" && type == Type::Struct);
	...
	{
		WINRT_ASSERT("Accessing T of non-array" && type == Type::Array);
...
#undef X
		...
		default:
			WINRT_ASSERT("Tried to get offset of empty or invalid Type::element" && false);
			return 0u;
		}
	}
	...
	{
		WINRT_ASSERT("Add to non-struct in layout" && type == Type::Struct);
		WINRT_ASSERT("invalid symbol name in Struct" && ValidateSymbolName(name));
		auto& structData = static_cast<ExtraData::Struct&>(*pExtraData);
		for (auto& mem : structData.layoutElements)
		{
			if (mem.first == name)
			{
				WINRT_ASSERT("Adding duplicate name to struct" && false);
			}
		}
		return structData.layoutElements.emplace_back(std::move(name), LayoutElement{ addedType }).second;
	}
	LayoutElement& LayoutElement::Set(Type addedType, size_t size) noxnd
	{
		WINRT_ASSERT("Set on non-array in layout" && type == Type::Array);
		WINRT_ASSERT(size != 0u);
		auto& arrayData = static_cast<ExtraData::Array&>(*pExtraData);
		arrayData.layoutElement = { addedType };
		arrayData.size = size;
		return *this;
	}
	LayoutElement& LayoutElement::Add(std::initializer_list<LayoutPair> pairs) noxnd
	{
		WINRT_ASSERT("Add to non-struct in layout" && type == Type::Struct);
		for (auto& pair : pairs)
		{
	...
	type{ typeIn }
	{
		WINRT_ASSERT(typeIn != Type::Empty);
...
#undef X
		...
		default:
			WINRT_ASSERT("Bad type in size computation" && false);
			return 0u;
		}
	}
	...
	size_t LayoutElement::FinalizeForStruct(size_t offsetIn)
	{
		...
		WINRT_ASSERT(data.layoutElements.size() != 0u);
	...
	{
		WINRT_ASSERT(&GetRootLayoutElement() == &other.GetRootLayoutElement());
