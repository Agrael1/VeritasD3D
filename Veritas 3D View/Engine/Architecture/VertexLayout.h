...
namespace DV
{
	...
	public:
		...
		static constexpr auto Bridge(VertexLayout::ElementType type, Args&&... args) noxnd
		{
			...
			WINRT_ASSERT("Invalid element type" && false);
	...
	public:
		...
		const Element& Resolve()const noxnd
		{
			...
			{
				WINRT_ASSERT(false && L"Couldn't resolve type name");
			}
	...
	private:
		...
		void SetAttribute(unsigned char* pAttribute, SrcType&& val) noxnd
		{
			...
			{
				WINRT_ASSERT("Parameter attribute type mismatch" && false);
			}
		}
	private:
	...
	public:
		...
		{
			WINRT_ASSERT(sizeof...(params) == layout.GetElementCount() && "Param count doesn't match number of vertex elements");
