	...
	tag(tag)
{
	...
	ver::check_hresult(GetDevice(gfx)->CreateBuffer(&bd, &sd, &pVertexBuffer));
}
...
VertexMultibuffer::VertexMultibuffer(Graphics& gfx, ver::dv::LayoutSpan lay, std::span<void*> data, size_t num_verts)
{
	...
	for (size_t i = 0; i < c; i++)
	{
		...
		sd.pSysMem = data[i];
		ver::check_hresult(GetDevice(gfx)->CreateBuffer(&bd, &sd, buffers[i].put()));
	}
}
void VertexMultibuffer::Bind(Graphics& gfx) noxnd
{
	...
	(GetContext(gfx)->IASetVertexBuffers(0u, uint32_t(buffers.size()), reinterpret_cast<ID3D11Buffer* const*>(buffers.data()), strides.data(), offsets));
	ver::check_context();
}
