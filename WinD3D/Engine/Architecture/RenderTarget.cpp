...
void RenderTargetArray::BindAsBuffer(Graphics& gfx) noexcept(!IS_DEBUG)
{
	ID3D11DepthStencilView* null = nullptr;
	BindAsTarget(gfx, null);
}

void RenderTargetArray::BindAsBuffer(Graphics& gfx, BufferResource* depthStencil) noexcept(!IS_DEBUG)
{
	assert(dynamic_cast<DepthStencil*>(depthStencil) != nullptr);
	BindAsBuffer(gfx, static_cast<DepthStencil*>(depthStencil));
}

void RenderTargetArray::BindAsBuffer(Graphics& gfx, DepthStencil* depthStencil) noexcept(!IS_DEBUG)
{
	BindAsTarget(gfx, depthStencil ? depthStencil->pDepthStencilView.Get() : nullptr);
}

void RenderTargetArray::Clear(Graphics& gfx) noexcept(!IS_DEBUG)
{
	Clear(gfx, { 0,0,0,0 });
}

void RenderTargetArray::Clear(Graphics& gfx, const std::array<float, 4>& color) noexcept(!IS_DEBUG)
{
	INFOMAN_NOHR(gfx);
	for (auto& i : targets)
	{
