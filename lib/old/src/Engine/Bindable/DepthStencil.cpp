...
namespace ver
{
		...
		height(height)
	{
		...
		ver::check_hresult(GetDevice(gfx)->CreateTexture2D(&descDepth, nullptr, pDepthStencil.put()));
		...
		ver::check_hresult(GetDevice(gfx)->CreateDepthStencilView(
	...
	{
		(GetContext(gfx)->PSSetShaderResources(slot, 1u, array_view(pShaderResourceView)));
		ver::check_context();
	}
